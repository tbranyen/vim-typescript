...
const entryFile = TS.sys.args[TS.sys.args.length - 1];
const config = join(process.cwd(), 'tslint.json');

// Run TSLint if found in the project.
try {
  execSync(`tslint --project ${config} ${entryFile}`);
}
catch (ex) {
  const errorStrings = ex.stdout.toString().split('\n');
  const regex = /(.*): (.*)\[(.*)\]: (.*)/g;
  ...
  errorStrings.forEach(errorString => {
    ...
    if (parts) {
      const [line, col] = parts[3].split(',');

      console.log(`${parts[2]}:${line}:${col}: ${parts[1]}: ${parts[4]}`);
    }
  });
}
